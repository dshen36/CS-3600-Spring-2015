        ...
        # and noisyDistance is None
            for p in self.legalPositions:
                ...
                if emissionModel[trueDistance] > 0:
                    ans[p] = emissionModel[trueDistance] * self.beliefs[p]
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                ans[newPos] += prob * self.beliefs[p]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        self.particleList = []
        chunk = self.numParticles / len(self.legalPositions)
        i = 0
        for x in range(1, self.numParticles + 1):
            pos = self.legalPositions[i]
            self.particleList.append(pos)
            if (x % chunk == 0) and ((i + 1) < len(self.legalPositions)):
                i = i + 1
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for j in range(0, self.numParticles):
                myList.append(self.getJailPosition())
            self.particleList = myList
        else:
            weight = util.Counter()
            i = 0
            ...
            if totalweight == 0:
                ...
                myList = []
                for key in keyList:
                    num = (int)((weight[key] / (float)(totalweight)) * self.numParticles)
                    for i in range(0, num):
                        if len(myList) < self.numParticles:
                            myList.append(key)
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        myDict = {}
        for loc in self.particleList:
            if loc in myDict.keys():
                myDict[loc] += 1
            else:
                myDict[loc] = 1
        ...
        for k in myDict.keys():
            ans[k] = myDict[k] / (float)(self.numParticles)
        return ans
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        while len(self.particles) < self.numParticles:
            for roll in product(self.legalPositions, repeat = 2):
                if len(self.particles) < self.numParticles:
                    self.particles.append(roll)
    ...
    def observeState(self, gameState):
        ...
        ** Remember ** We store particles as tuples, but to edit a specific particle,
        ...
        for particle in self.particles:
            ...
            particleWeight = 1
            for i in range(self.numGhosts):
                if i not in ghostInd:
                    trueDistance = util.manhattanDistance(particle[i], pacmanPosition)
                    particleWeight = particleWeight * emissionModels[i][trueDistance]
            weight[particle] += particleWeight
        ...
        if totalweight == 0:
            ...
            weight.normalize()
            for key in keyList:
                num = (int)(weight[key] * self.numParticles)
                for i in range(0, num):
                    if len(myList) < self.numParticles:
                        myList.append(key)
    ...
    def elapseTime(self, gameState):
        ...
        **Note** that you may need to replace "prevGhostPositions" with the
        ...
        for oldParticle in self.particles:
            ...
            mylist = list(oldParticle)

            for i in range(0, self.numGhosts):
                dist = getPositionDistributionForGhost(setGhostPositions(gameState, mylist), i, self.ghostAgents[i])
    ...
    def getBeliefDistribution(self):
        ...
        ans = util.Counter()
        for particle in self.particles:
            ans[particle] += 1 / (float)(self.numParticles)
        return ans
