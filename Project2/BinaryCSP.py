...
class nQueenConstraint(BinaryConstraint):
	def isSatisfied(self, value1, value2):
...
def consistent(assignment, csp, var, value):
	"""Question 1"""
	"""YOUR CODE HERE"""
	assignedValuesDict = assignment.assignedValues
	for i in csp.binaryConstraints:
		if i.affects(var):	
			ov = i.otherVariable(var)
			otherVal = assignedValuesDict[ov]
			if not i.isSatisfied(value, otherVal):
				return False
	return True
...
def recursiveBacktracking(assignment, csp, orderValuesMethod, selectVariableMethod):
	...
	orderValues = orderValuesMethod(assignment, csp, var)
	for val in orderValues:
...
def minimumRemainingValuesHeuristic(assignment, csp):
	...
	"""Question 2"""
	"""YOUR CODE HERE"""
	constraintList = csp.binaryConstraints
	varValMap = assignment.assignedValues
	unassignedList = []
	for i in varValMap.keys():
		if varValMap[i] is None:
			unassignedList.append(i)

	minVars = 0
	minDomainLen = len(domains[unassignedList[0]])
	for m in domains[unassignedList[0]]:
		if consistent(assignment, csp, unassignedList[0], m):
			minVars = minVars + 1
	nextVar = unassignedList[0]

	for j in unassignedList:
		if len(domains[j]) < minDomainLen:
			nextVar = j
			minDomainLen = len(domains[j])
		else:
			if len(domains[j]) == minDomainLen:
				count = 0
				for k in domains[j]:
					if consistent(assignment, csp, j, k):
						count = count + 1
				if count < minVars:
					minVars = count
					nextVar = j
					...
					if count == minVars: 
						...
						for c in constraintList:
							if c.affects(j):
								otherVal = c.otherVariable(j)
								if otherVal in unassignedList:
									degree = degree + 1
							if c.affects(nextVar):
								otherVal = c.otherVariable(nextVar)
								if otherVal in unassignedList:
									degree2 = degree2 + 1
						if degree > degree2:
							nextVar = j
...
def leastConstrainingValuesHeuristic(assignment, csp, var):
	...
	tupList = []
	for value in values:
		count = acceptableValCounter(assignment, csp, var, value)
		if len(tupList) == 0:
			tupList.append((value, count))
		else:
			ind = 0
			while ind < len(tupList) and count < tupList[ind][1]: #what if tie
				ind = ind + 1
			tupList.insert(ind, (value, count))
...
def acceptableValCounter(assignment, csp, var, value):
	if not consistent(assignment, csp, var, value):
		count = 0
		for i in csp.binaryConstraints:
			if i.affects(var):
				other = i.otherVariable(var)
				if assignment.assignedValues[other] == value:
					count = count - 1
		return count

	myMap = {}
	for k in assignment.assignedValues.keys():
		myMap[k] = assignment.assignedValues[k]
	domains = assignment.varDomains
	myMap[var] = value
	count = 0
	for i in csp.binaryConstraints:
		if i.affects(var):
			other = i.otherVariable(var)
			if myMap[other] is None:
				for temp in domains[other]:
					if consistentH(myMap, csp, other, temp, var):
						count = count + 1
	return count



def consistentH(assignedValuesDict, csp, var, value, targetVar):
	for i in csp.binaryConstraints:
		if i.affects(var):
			ov = i.otherVariable(var)
			otherVal = assignedValuesDict[ov]
	...
	for i in csp.binaryConstraints:
		if i.affects(var):
			...
			for val in assignment.varDomains[other]:
				if not i.isSatisfied(value, val):
					removedVal = val
					break
			if removedVal is not None:
				assignment.varDomains[other].remove(removedVal)
				inferences.add((other, removedVal))
				if len(assignment.varDomains[other]) == 0:
					#reverse
					for tup in inferences:
						assignment.varDomains[tup[0]].add(tup[1])
					return None
...
def maintainArcConsistency(assignment, csp, var, value):
	...
	for i in csp.binaryConstraints:
		if i.affects(var):
			other = i.otherVariable(var)
			dq.append([var, other, i])

	inconsistent = False
	while len(dq) != 0:
		elmnt = dq.popleft()
		inf = revise(assignment, csp, elmnt[0], elmnt[1], elmnt[2])
		if inf is None:
			inconsistent = True
			break
		else:
			if len(inf) == 0:
				...
				inferences.update(inf)
				for i in csp.binaryConstraints:
					if i.affects(elmnt[1]):
						other = i.otherVariable(elmnt[1])
						if other != elmnt[0]:
							dq.append([elmnt[1], other, i])
...
def AC3(assignment, csp):
	...
	"""YOUR CODE HERE"""
	dq = deque()
	varList = assignment.varDomains.keys()
	for var in varList:
		for i in csp.binaryConstraints:
			if i.affects(var):
				other = i.otherVariable(var)
